...
while true
    ...
    if sourceNode < 1 || sourceNode > numNodes || destinationNode < 1 || destinationNode > numNodes
        ...
        if changeWeights
            changeFromNode = input('Enter the source node to change weight: ');
            changeToNode = input('Enter the destination node to change weight: ');
            newWeight = input('Enter the new weight: ');

            if changeFromNode >= 1 && changeFromNode <= numNodes && changeToNode >= 1 && changeToNode <= numNodes
                % Update weights in both directions
                adjacencyMatrix(changeFromNode, changeToNode) = newWeight;
                adjacencyMatrix(changeToNode, changeFromNode) = newWeight;
